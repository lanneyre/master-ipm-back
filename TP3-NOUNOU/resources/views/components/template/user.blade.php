...
@section('content-' . $user->id)
    <form action="{{ route('user.updateme', $user->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        ...
        <!-- Mot de passe -->
        <div>
            <label for="password" class="block font-medium text-gray-700">Mot de passe (laisser vide pour ne pas
                changer)</label>
        ...
        <div class="text-center">
            <button type="submit" class="² text-white px-4 py-2 rounded-lg hover:bg-indigo-700">
                Mettre à jour
            </button>
        </div>
    </form>
